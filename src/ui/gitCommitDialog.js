// File: src/ui/gitCommitDialog.js
/**
 * Git commit dialog
 * Handles the process of committing a document to Git (GitHub or Gitea)
 */
...
export class GitCommitDialog {
  ...
  hide() {
    ...
    console.log('Git commit dialog closed');
  }
    ...
    modal.innerHTML = `
      <div class="modal-dialog git-commit-dialog">
        <div class="modal-header">
          <h2 id="commit-title" class="modal-title">Commit to ${platformName}</h2>
        ...
        <div class="modal-content">
          <div class="settings-section">
            ...
            <div class="input-group">
              <label for="commit-repo">${platformName} Repository:</label>
        ...
        <div class="modal-footer">
          <button id="configure-git" class="secondary-button">Configure ${platformName}</button>
          <button id="execute-commit" class="primary-button">Commit to ${platformName}</button>
  ...
  setupEventListeners() {
    ...
    const configButton = document.getElementById('configure-git');
    if (configButton) {
  ...
  /**
   * Execute commit to Git repository
   */
  async executeCommit() {
    const repoSelect = document.getElementById('commit-repo');
    const pathInput = document.getElementById('commit-path');
    ...
    this.setStatus('loading', `Committing to ${gitService.getActivePlatformName()}...`);
    ...
    try {
      ...
      gitService.settings.commitMessage = messageInput.value.trim();
      gitService.saveSettings();
      
      this.setStatus('success', `Successfully committed to ${gitService.getActivePlatformName()}!`);
      
      // Create link to view on Git platform
      const commitUrl = result.commit?.html_url || gitService.settings.lastCommit?.url;
      if (commitUrl) {
        const statusEl = document.getElementById('commit-status');
        if (statusEl) {
          const viewLink = document.createElement('a');
          viewLink.href = commitUrl;
          viewLink.target = '_blank';
          viewLink.textContent = `View on ${gitService.getActivePlatformName()}`;
          viewLink.className = 'git-link';
...
export const gitCommitDialog = new GitCommitDialog();

// Make available globally for menu system
window.gitCommitDialog = gitCommitDialog;
